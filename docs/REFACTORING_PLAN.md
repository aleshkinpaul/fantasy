...
> Статус на 28 августа 2026 года: этапы characterization tests, типизации границы, централизованной загрузки, domain-расчетов, facade, конфигурации сезонных правил, удаления `@ts-nocheck` и error view актуальной страницы выполнены. Данные 2025–26 совпадают с golden-файлами. Остаются разбиение сезонных файлов, устранение дублирования `matches`/`schedule` и дополнительные unit/UI-тесты.
...
- актуальные разделы 2025–26 в `profiles.json` и `consts.json`;
...
- разделить монолитный `consts.json` по сезону и соревнованию либо добавить manifest;
- генерировать `schedule` из единственного источника `matches`;
