...
Workflow сравнивает статические файлы по размеру (Beget не всегда сохраняет исходное время изменения), а данные турниров из `assets/data` обновляет принудительно. `index.html` загружается последним, чтобы во время обновления он не ссылался на ещё не переданные bundle-файлы; затем опубликованный `deploy-version.txt` сверяется с SHA коммита. Старые хешированные файлы автоматически не удаляются, поэтому деплой не затрагивает вручную созданные файлы на сервере.
