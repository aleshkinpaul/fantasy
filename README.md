...
3. Если FTP-аккаунт открывается не сразу в `public_html`, добавьте Repository variable `BEGET_FTP_PATH` с путём назначения. Для SFTP путь считается от домашнего каталога аккаунта, поэтому значения `fr-fantasy.ru/public_html` и `/fr-fantasy.ru/public_html` обрабатываются одинаково. Для аккаунта, привязанного к каталогу сайта, переменную можно не создавать: используется `/`.

Workflow сравнивает статические файлы по размеру (Beget не всегда сохраняет исходное время изменения), а данные турниров из `assets/data` обновляет принудительно. На шаг выгрузки установлен лимит 10 минут. `index.html` загружается последним, чтобы во время обновления он не ссылался на ещё не переданные bundle-файлы; затем опубликованный `deploy-version.txt` сверяется с SHA коммита. Старые хешированные файлы автоматически не удаляются, поэтому деплой не затрагивает вручную созданные файлы на сервере.
